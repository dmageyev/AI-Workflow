...
- Orchestrator читає [`.ai/memory/state/project-state.yaml`](../memory/state/project-state.yaml).
- Orchestrator читає останній handoff у [`.ai/logs/sessions/`](../logs/sessions/) (сортування за назвою папки).
...
- Усі нові рішення фіксуються ADR у [`.ai/memory/decisions/`](../memory/decisions/).

### 4. Завершення сесії (Session Close)

- Оновити статуси work items у [`.ai/memory/state/project-state.yaml`](../memory/state/project-state.yaml).
- Створити Handoff Package:
  - [`.ai/logs/sessions/YYYY-MM-DD-HHmm-topic/snapshot.yaml`](../logs/sessions/)
  - [`.ai/logs/sessions/YYYY-MM-DD-HHmm-topic/handoff.md`](../logs/sessions/)
- Перевірити quality gates ([`.ai/docs/04-quality-gates.md`](04-quality-gates.md)).
