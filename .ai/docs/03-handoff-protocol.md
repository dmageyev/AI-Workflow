...
1. **Детекція незакритої сесії.** Orchestrator на початку перевіряє: чи існує папка в
   `.ai/logs/sessions/`, яка не містить обох файлів (`snapshot.yaml` та `handoff.md`).
...
3. **Відновлення контексту.** Наступна сесія повинна:
   - Прочитати `project-state.yaml` як canonical state
...
|---|---|---|
| **Розташування** | `.ai/logs/sessions/<session>/` | `.ai/memory/state/` |
