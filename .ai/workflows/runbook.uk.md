...
## 7. Вирішення merge-конфліктів

Конфлікти в цьому репозиторії найчастіше виникають у файлах:
`.ai/memory/state/project-state.yaml`, `CHANGELOG.md`, `llms.txt`.

### Стратегія вирішення

| Файл | Стратегія |
| ---- | --------- |
| `project-state.yaml` | Зберігай **вищу** версію (`version`), об'єднуй `work_items` з обох гілок |
| `CHANGELOG.md` | Додавай записи з обох гілок, зберігаючи хронологічний порядок |
| Інші `.md` файли | Вирішуй вручну, зберігаючи зміст обох гілок |

### Кроки

1. Ідентифікуй конфліктні файли: `git status`
2. Відкрий кожен файл та вирішуй конфлікти вручну між маркерами `<<<<<<<` / `=======` / `>>>>>>>`
3. Після вирішення: `git add <файл>`
4. Завершуй merge: `git commit`
5. Запусти CI локально або перевір GitHub Actions після push

### Превентивні заходи

- Синхронізуй гілку з `main` перед початком нової сесії: `git fetch origin && git rebase origin/main`
- Не тримай довгоживучі гілки — merge якомога швидше після завершення роботи
- Якщо конфлікт у `project-state.yaml` складний — залуч Orchestrator для ухвалення рішення
